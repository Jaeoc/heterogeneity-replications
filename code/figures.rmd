...
Figure 1. Result of simulation relating $I^2$-values to between studies standard deviation. Each line represent one of 67 effects. Tau is not directly comparable across effect size measures. 
...
Figure 2. Simulated $I^2$  densities across 67 effects for zero, small, medium, and large heterogeneity according to the definitions of Higgins (2003), and a histogram of the observed $I^2$ estimates for the 67 effects. Each simulated density consists of approximately 670,000 estimates. 
...
log_or_fit <- function(x){ #NB! if a cell is 0, 1/2 will be added to it
...
log_odds_ratios <- dat %>% 
  ...
  split(.$effect) %>% 
  map_dfr(log_or_fit, .id = "effect")

#One with NA values: ML2; Trolley dilemma 1; Site = 'cas'
...
Figure 3. The correlation between absolute effect size and A) tau2, B) I2 and C) H2 respectively for 40 effects that were measured as mean or standardized mean differences and 10 effects measured as odds ratios or risk differences from 13 pre-registered multi-lab replication projects. Effects reported as mean differences were standardized and risk differences computed as log odds ratios. Square brackets contain 95% bootstrapped percentile confidence intervals.
