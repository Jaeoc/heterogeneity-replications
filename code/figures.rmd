...
# #Effects estimated with sourced function 'summarizer'
# observed <- dat %>% 
#   split(.$effect) %>%  
#   map_dfr(summarizer, .id = "effect") %>% 
#   select(effect, tau2 = tau2) %>%
#   mutate(Heterogeneity = "Observed")
# 
# tau2_plot <- ggplot(tau2_dist, aes(x = tau2, group = Heterogeneity, fill = Heterogeneity, linetype = Heterogeneity)) +
#   geom_histogram(data = observed, aes(y = ..density.., x = I2), bins = 100, alpha = 1) +
#   geom_density(alpha = 0.3) +
#   theme_classic() +
#   coord_cartesian(xlim = c(0, 100))  +
