...
#Basic correlations
cor.test(res$I2, res$r) #pearson I2

I2_fit <- summary(lm(I2 ~ r, data = res)) #for annotating figures

cor.test(res$tau2, res$r) #pearson tau2

tau2_fit <- summary(lm(tau2 ~ r, data = res))

cor.test(res$H2, res$r) #pearson H2

H2_fit <- summary(lm(H2 ~ r, data = res))

cor.test(res$I2, res$r, method = "spearman") #spearman

##Excluding anchoring effects
dropped_anchoring <- res %>%
...
bootfit <- boot(data=res, statistic=pear, R=1000, formula= ~ r + I2)
...
options(warn=-1)
bootfit <- boot(data=res, statistic=spear, R=1000, formula= ~ r + I2)
options(warn=0)
...
bootfit <- boot(data=res, statistic=pear, R=1000, formula= ~ r + H2)
...
pear <- function(formula, data, indices){
     d <- data[indices,]
     fit <- cor.test(formula = ~ tau2 + r, data=d)
     return(fit$estimate)
}

bootfit <- boot(data=res, statistic=pear, R=1000, formula= ~ r + tau2)
...
tau2 <- ggplot(res, aes(x = r, y = tau2)) + 
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
...
#If we don't use the Odds ratios we don't need the below
