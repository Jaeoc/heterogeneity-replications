...
# Observed tau22 estimates, i.e, results from main table, see table.rmd
...
tau2_plot <- ggplot(tau2_dist, aes(x = tau2_hat, group = Heterogeneity, fill = Heterogeneity, linetype = Heterogeneity)) +
  geom_histogram(data = observed_tau2, aes(y = ..density.., x = tau2), bins = 200, alpha = 1) +
  geom_density(alpha = 0.3) +
  theme_classic() +
  ...
  theme(legend.position = c(0.85, 0.71),
       legend.background = element_rect(linetype = "solid", color = "black")) +
  scale_y_continuous(name = "Density") +
  scale_x_continuous(name = expression(paste(italic(tau)^2," index", sep = ""))) +
  guides(fill = guide_legend(override.aes = list(alpha = c(rep(0.3, 4), 1))))

#ggsave("../figures/density-tau2.pdf", dpi = 600, height =  4, width = 6.47, device = cairo_pdf())

#3) combine I2-tau2-density-plots----
densities <- plot_grid(I2_plot, tau2_plot, nrow = 2, labels = c("a", "b"), align = "v") #combine plots
densities
...
#compute random effect meta-analysis of transformed effects
...
#Plots of H2 and I2 vs. biserial correlation r for all effects together

I2 <- ggplot(res2, aes(x = r, y = I2)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
  coord_cartesian(ylim = c(0, 100)) +
  geom_text(x = 0.14, y = Inf, vjust = 7, fontface = "italic", size = 3.5, label = paste0("b = ", format(round(I2_fit$coefficients[2, 1], 2), nsmall = 2), ", r = ", format(round(sqrt(I2_fit$r.squared), 2), nsmall = 2))) +
  theme_classic() +
  scale_y_continuous(name = expression(paste(italic(I)^2," index"))) +
  scale_x_continuous(name = expression(paste("Biserial and pearson correlations ")))

H2 <- ggplot(res2, aes(x = r, y = H2)) + 
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
  geom_text(x = 0.14, y = Inf, vjust = 7, size = 3.5, fontface = "italic", label = paste("b = ", format(round(H2_fit$coefficients[2, 1], 2), nsmall = 2), ", r = ", format(round(sqrt(H2_fit$r.squared), 2), nsmall = 2))) +
  theme_classic() +
...
##Alternative/additional Figure 3
```{r r_vs_tau2, fig.asp = 0.618, cache = TRUE}
r_tau2 <- ggplot(res2, aes(x = r, y = tau2)) + 
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = TRUE, color = "black") +
  geom_text(x = 0.14, y = Inf, vjust = 7, size = 3.5, fontface = "italic", label = paste("b = ", format(round(tau2_fit$coefficients[2, 1], 2), nsmall = 2), ", r = ", format(round(sqrt(tau2_fit$r.squared), 2), nsmall = 2))) +
  theme_classic() +
