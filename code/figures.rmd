...
Should I first run MA and then transform effect sizes? ->Not sure how how to treat the raw mean differences, since I only get the MA-estimate estimate and the SE.  More straightforward to convert to common metric first. **Kolden + Ihle give formulas.** From book Cooper & Hedges, 1994.
...
summarizer <- function(x){
  ...
  data.frame(z = fit$b[[1]], I2 = fit$I2, H2trunc = fit$H2, H2 = fit$QE / fit$k) #estimate out
}
...
effect_het <- plot_grid(I2, H2, nrow = 2, labels = c("A", "B"), align = "v")
