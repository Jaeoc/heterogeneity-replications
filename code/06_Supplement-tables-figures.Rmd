...
This document contains the tables and figures from Supplement A, B and C for the project "Heterogeneity in direct replications in psychology and its association with effect size". 
...
c_plot <- ggplot(d, aes(x = I2, y = sqrt(tau2))) + 
  geom_point() + 
  scale_y_continuous(name = "Tau") +
  scale_x_continuous(name = expression(paste(italic(I)^2, " Index"))) +
  ggtitle("Point-Biserial r (transformed SMD)")
...
b <- select(a, effect, tau2) %>%
  left_join(select(d, effect, tau2), by = "effect") #add point-biserial transformation tau2 to untransformed
```
