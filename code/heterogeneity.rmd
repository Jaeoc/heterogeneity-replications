...
est_heterogen_smd_raw <- function(x){
  if(any(x[, "outcomes1_2"] == "mean _ SD")){ #without the 'any' we will get warnings because we apply 'if' to a vector
    ...
    fit <-rma(measure = "ZCOR", ri = effect_size, ni = Ntotal, test = "knha", data = x) #Fisher's r-to-z transformed correlation coefficient
    fit2 <- rma(measure = "COR", ri = effect_size, ni = Ntotal, test = "knha", data = x) #raw correlation
...
clarify <- c("Allowed vs. forbidden", "Gain vs. loss framing", "Norm of reciprocity", "Low vs. high category scales")

#-----------TEST
test <- dat %>% filter(effect == "IAT correlation math")
fit <- rma(measure = "ZCOR", ri = effect_size, ni = Ntotal, test = "knha", data = test)
fit2 <- rma(measure = "COR", ri = effect_size, ni = Ntotal, test = "knha", data = test)
