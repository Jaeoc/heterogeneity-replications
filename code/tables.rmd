...
rooted_relabilities %>% knitr::kable("latex", booktabs = T, col.names = c("", "$\\rho_{xy}$ = 0", "$\\rho_{xy}$ = .3", "$\\rho_{xy}$ = .5"), escape = FALSE, digits = 2) %>% 
  ...
  footnote(threeparttable = TRUE, escape = FALSE,
           general = "Values in cells are observed effect sizes arising from the true effect size $\\\\rho_{xy}$ and measurement reliabilites $\\\\sqrt{R_{xx'}}\\\\times\\\\sqrt{R_{yy'}}$ when sample size is infinite. Increasing differences between observed effect sizes in the columns illustrate the association between effect size, reliability of measurement and heterogeneity.  Code to reproduce table: osf.io/kf6pt/") %>% 
...
summary.table %>% knitr::kable("latex", 
                               ...
                               longtable = TRUE) %>% 
  kable_styling(c("hold_position", "repeat_header"), font_size = 7) %>% 
...
#'summarizer' is a sourced function
res2 <- dat2 %>% 
  split(.$effect) %>%  #separate by effect, necessary step otherwise the function is applied overall
