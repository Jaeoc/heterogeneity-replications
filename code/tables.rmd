---
title: "Tables"
author: "Limited evidence for widespread heterogeneity in psychology"
output: pdf_document
---
...
diff_row <- data.frame(rel = "SD observed effect sizes", X1 = sd(rooted_relabilities$X1), X2 = sd(rooted_relabilities$X2), X3 = sd(rooted_relabilities$X3))

het_demo <- rbind(rooted_relabilities, diff_row)

het_demo %>% knitr::kable("latex", booktabs = T, col.names = c("", "$\\rho$ = 0", "$\\rho$ = .3", "$\\rho$ = .5"), escape = FALSE, digits = 2) %>% 
  kable_styling(position = "left") %>% 
  footnote(threeparttable = TRUE, escape = FALSE,
           general = "Values in cells are observed effect sizes arising from the true effect size $\\\\rho$ and measurement reliabilites $\\\\sqrt{R_{xx'}}\\\\times\\\\sqrt{R_{yy'}}$. SD = standard deviation. Code to reproduce table: osf.io/kf6pt/") %>% 
...
het <-  het %>% 
  ...
  mutate(ci = paste0("[", trimws(format(round(s_ci.lb, 2), nsmall = 2)), ", ", trimws(format(round(s_ci.ub, 2), nsmall = 2)), "]")) %>% 
  select(rp, effect, k, effect_type, eff_size, s_I2, ci, zero, small, medium, large)
