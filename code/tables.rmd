...
res <- res %>%
  mutate(tau = sqrt(res2$tau2), #For table 3, report tau and transformed effect sizes (biserial correlations)
         tau_ci.lb = sqrt(res2$tau2.lb),
         tau_ci.ub = sqrt(res2$tau2.ub),
         r = res2$r)
...
het <-  het %>% 
  ...
  mutate(I2_ci = paste0("[", trimws(format(round(s_ci.lb, 2), nsmall = 2)), ", ", trimws(format(round(s_ci.ub, 2), nsmall = 2)), "]"), #Round and put confidence interval between brackets, e.g., [0, 0.41]
         tau_ci = paste0("[", trimws(format(round(tau_ci.lb, 4), nsmall = 4)), ", ", trimws(format(round(tau_ci.ub, 4), nsmall = 4)), "]"),
         tau = format(round(tau, 4), nsmall = 4)) %>% #rounded here instead of in table function because should have 4 digits
  select(rp, effect, k, effect_type, eff_size, s_I2, I2_ci, r, tau, tau_ci, zero, small, medium, large)
...
het %>% knitr::kable("latex", align = "c", booktabs = TRUE, escape = FALSE, digits = 2, col.names = c("RP", "Effect", "K", "Effect type", "Effect size estimate",  "$I^2$(\\%)", "$I^2$ 95\\% CI", "r*" , "$\\tau$", "$\\tau$ 95\\% CI", "Zero", "Small", "Medium", "Large")) %>% 
