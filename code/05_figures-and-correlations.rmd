...
#tau plot----
tau_facet <- ggplot(tau2_per_type, aes(x = eff_size, y = sqrt(tau2))) + #NB! Taking sqrt of tau2 here
...
#bootfit <- boot(data=SMDs, statistic=pear, R=1000, formula= ~ eff_size + s_I2) #see Fig 3
...
# bootfit <- boot(data=tau2_per_type, statistic=pear, R=1000, formula= ~ eff_size + tau2) #see Fig 3
...
set.seed(423)
bootfit <- boot(data=dropped_anchoring, statistic=pear, R=1000, formula= ~ eff_size + tau)
...
set.seed(5233)
tau2_per_type <- tau2_per_type %>% mutate(tau = sqrt(tau2))
options(warn=-1)
bootfit <- boot(data=tau2_per_type, statistic=spear, R=1000, formula= ~ eff_size + tau)
options(warn=0)
